...
window.onload = function() {
    ...
    passcodeBtn.addEventListener('click', function() {
        ...
        if (p && p.length >= 4) {
            ...
            chrome.storage.local.set({'passcode': pUpper}, function() {
                passcode = pUpper;
                alert('Passcode changed to: ' + pUpper);
            });
        } else if (p !== null) {
        ...
        chrome.runtime.sendMessage({
            ...
            for (var i = 0; i < response.length; i++) {
                ...
                recordList.innerHTML += '<li>' + escapeHtml(row.user) + ' &#8227; ' +
                    escapeHtml(row.pass) + ' <a href="' + escapeHtml(row.href) + '" target="_blank" rel="noopener noreferrer">' +
