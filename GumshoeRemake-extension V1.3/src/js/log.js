(function () {
        ...
        if (userInput && passInput && userInput.value && passInput.value) {
          chrome.runtime.sendMessage({
            ...
            if (chrome.runtime.lastError) {
              console.warn('[content script] Error sending message:', chrome.runtime.lastError.message);
            } else if (!response || !response.success) {
              console.warn('[content script] No response or operation unsuccessful.');
            }
          });
        }
      } catch (e) {
        console.error('[content script] Error while handling form:', e);
      }
    });
  }
  ...
  window.addEventListener('keydown', function (event) {
    ...
    if (char === passcode.charAt(progress)) {
      progress++;
      if (progress === passcode.length) {
        console.log('[content script] Passcode matched in input! Opening manage.html...');
